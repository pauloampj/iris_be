...
Use Damaplan\Iris\Core\Network\DMPLRequest;
Use Damaplan\Iris\Core\Network\DMPLResponse;
...
class DMPLApi {
	...
	public function _getVersionPosition(){
		$uri = $this->_request->getUri();
		
		if($uri !== false){
			$pieces = explode('/', $uri);
			
			foreach($pieces as $pos => $p){
				if(is_numeric($p)){
					return $pos;
				}
			}
